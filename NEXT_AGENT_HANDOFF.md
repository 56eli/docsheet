...
python -m unittest discover tests     # 82 tests, offline, ~2s
...
| Test suite | **82 tests; coverage 92% total, every pipeline module ≥ 88%** | `.coveragerc` enforces `fail_under = 80` |
...
7. **Tests + fail-safes (this turn):**
   - `tests/test_pipeline.py` — 82 tests: end-to-end write/check/tamper runs of
...
9. **Independent re-audit + doc-status pass (this turn, branch
   `arena/019fc893-docsheet`):** verified every previous claim (82 tests,
