...
class ViewsConfigConsistencyTests(unittest.TestCase):
    ...
    def test_views_covers_every_user_facing_catalogue_output(self) -> None:
        ...
        # data.json is the raw pass-through output, not a bcp OUT_*; it is still
        # generated by process_data.py but is no longer exposed as a Jump-to
        # view (the Original Spreadsheet view was retired 2026-08-10), so treat
        # it as non-user-facing alongside the meta/manifest files.
        non_viewing = {"catalogue-meta.json", "catalogue-block-map.json", "build-manifest.json", "data.json"}
