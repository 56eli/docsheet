...
test('Master ID column sorts numerically, not lexically', async ({ page }) => {
  ...
  // The uuid column sits behind the frozen title/filename rail; force the click
  // so the frozen subtitle does not intercept pointer events.
  await uuidHeader.click({ force: true });
