...
## Additional session work (owner-directed)

After the P3 cleanup, the owner directed three further changes; all are on this branch and verified green.

### Mobile bloat reduction (P1–P4, mobile-only)
The mobile catalogue (Everything) Browse view opened with ~430px of chrome before the first card on a ~640–740px screen. Four mobile-only (`≤720px`) changes cut it ~55% so the first card lands in the first viewport, with desktop (`≥721px`) untouched: (P1) a single-row topbar via icon-only Export + a new Settings gear icon, no "Jump to" label, and a shrinkable search; (P2) a compact, dismissible Browse intro (persists per browser); (P3) the Series/Timeline discovery rails collapsed behind a "Filter by series / year" toggle that auto-expands when a facet is active; (P4) a condensed view summary (the redundant Type card is hidden, spacing tightened). Plan: `review/MOBILE_BROWSE_PLAN.md`.

### Retire the Original Spreadsheet view
The "Original Spreadsheet" Jump-to entry (raw `data.json` pass-through) is removed from the site entirely — deleted from `config.js` (VIEWS/VIEW_GROUPS/VIEW_DETAILS/COLUMN_PRESETS), its blank-rows settings UI + `app.js` handling + dead CSS removed, `blank-rows.spec.js` deleted, and the two specs that referenced it repointed to other views. `data.json` is still generated by the raw lane (`process_data.py`) but is no longer surfaced; `ViewsConfigConsistencyTests` now treats it as non-user-facing. README's "All 19 … Jump to dropdown" still holds (19 entries remain).

### Owned-flag edits
- Master 373 (`manual-veritas-pvf-old-hardcover`, Power vs. Force Original Hardcover): `owned` blanked.
- Every ledger ITEM row with `raw_row_number ≥ 297` (41 rows): `owned` blanked.
Both edits were made in the reviewed source (`manual_master_candidates.csv`, `migration_review_ledger.csv`) — never the generated master — then rebuilt. Ownership distribution: **312 true / 25 false / 26 blank → 282 true / 13 false / 68 blank**.

### Data-integrity answers (links / Vimeo / owned)
Documented in `review/LINKS_VIMEO_OWNED_FINDINGS.md`: (1) the curated catalogue carries only the five official/legal source domains (Veritas, Hay House, Amazon, Audible, Nightingale-Conant), all HTTPS, zero filesharing — a few non-product goodreads/discord/github links exist only in the verbatim raw view; (2) the "missing" Vimeo links were Veritas-hosted OTR lecture trailers that Veritas removed — never carried into the catalogue; (3) `owned` is reviewer-set with only partial confirmation (16 archive.org-verified) and a specific unresolved mismatch is open in issue #18 (blocked on owner Drive access).

## Final final assessment

The branch is release-ready: the live site is repaired and byte-verified, mobile chrome is cut ~55%, the raw view is retired per the owner's call, the requested `owned` flags are blanked, and the full suite is green (149/149 tests, 3/3 Node, 6/6 `--check` modes, 90% coverage, all browser specs valid). Remaining items are owner-only (CI-gated Pages, visual acceptance, issue #18 Drive access).
