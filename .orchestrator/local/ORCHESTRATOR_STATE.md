...
| 006 | .orchestrator/prompts/006-proposed-filename-underscores.md | Adopt underscore convention for proposed_filename columns | arena/01a0ce73-docsheet (session-locked) | #79 | Merged 2026-09-23 (main `c6e865b`); MERGE reviewed same day; underscore standard live on main (363 rows, exemplar uuid 7 exact); crosscheck buckets unchanged; 2 Session Irregularities triaged Scoped |

## Active Milestone

Owner-directed phase: task **006 closed** — PR #79 merged 2026-09-23 (`main` `c6e865b`); underscore standard live across proposal sheet, master, Pages payloads. Scoped unfreeze ended with the merge — general research byte-freeze stands. Task 005 closed (PR #78, `1f7dab4`); buckets D/E await owner rulings. Maintenance CLOSED 2026-09-22 (PRs #74–#77). Awaiting owner's next directive.
...
- [x] 006: Underscore convention for `proposed_filename` (both columns; collapse ` - ` → `_`, then spaces → `_`) — PR #79 **merged 2026-09-23** (`main` `c6e865b`); MERGE reviewed same day; all transform invariants proven, buckets held, disclosed 2-line report regen was §4.5-compliant; 2 irregularities triaged Scoped
- [ ] OPEN THREADS for owner: bucket D (15 owned=true works, no registry file) + bucket E (41 stale owned flags) rulings from 005 remain open; research phase still byte-frozen (scoped 006 unfreeze closed with the merge)
...
- 2026-09-23 — ANTI-DEGRADATION FRESHNESS (milestone: owner-directed task 006 closed — PR #79 merged, `main` `c6e865b`): CORE sha256 `37b4c4254aa0e2cb418431a28d282041f233c442a1411f20024ea4fa2c697d52` matches tracker anchor in a fresh clone (duty 3); five-duty gate **8/8 OK** online against merged `main` (duty 5 live); CI SUCCESS on `c6e865b` (includes Playwright e2e); suite 213; `crosscheck --check` exit 0; underscore standard verified live (proposal row uuid 1 + master.json, 0 spaced values); forbidden surfaces (raw lane, registry, tracker, workflows) byte-clean vs pre-merge main. Discipline re-quoted from tracker §1/§2: research byte-freeze restored after scoped 006 unfreeze (proposal sheet + generator outputs only, merge closed it), scoreboard ban intact, CORE anchor authority intact, distribution shape intact, one-agent-at-a-time held (006 singular). Known local-only gap unchanged: sandbox lacks pandas (8 env failures; CI authoritative).

## Interrupted Work

None. (001–006 → PRs #74–#79 all merged; open threads = owner rulings on buckets D/E.)
...
- Live: shallow clone (boot fetches are depth-1 — deepen per operation as this session already does); duty-5 gate baseline `df85a20` must stay inside the depth-50 window (revisit ~40 orch publishes, fail-closed message guides); no express PR-authorization quotes exist (none given, none needed yet); session `GH_TOKEN` can expire mid-task (orchestrator publishes + PR #78 authoring, 2026-09-22) — halt, report, request operator reconnect; never route around an auth failure; sandbox cannot download Playwright browsers (`cdn.playwright.dev` TLS-reset, seen PR #79) — e2e specs must run in CI (workflow installs Chromium + `npm run test:e2e`); `gh api` outside repo scope returns 403 on repo-scoped tokens (expected — git push/PR flows unaffected).
...
| 2026-09-23 | 006 | Environment | PR #79: Playwright browser download from `cdn.playwright.dev` TLS-reset (ECONNRESET) inside sandbox — e2e specs not runnable locally | local e2e gap only; specs reviewed by inspection + run green in CI (workflow installs Chromium, `npm run test:e2e` in the SUCCESS run) | Scoped | known sandbox egress limitation; CI remains the e2e authority — no code impact |
| 2026-09-23 | 006 | Environment | PR #79: `gh api user` → 403 (repo-scoped token) | profile endpoint unreachable; git push + PR creation unaffected | Scoped | expected credential scope; no workaround needed — nothing routes around it |
