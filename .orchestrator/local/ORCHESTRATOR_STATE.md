...
| 006 | .orchestrator/prompts/006-proposed-filename-underscores.md | Adopt underscore convention for proposed_filename columns | arena/01a0ce73-docsheet (session-locked) | #79 | Reviewed 2026-09-23 — **MERGE** advised (awaiting operator merge); CI SUCCESS incl. Playwright e2e; field audit 363/363 rule-exact, untouched cols identical, uniqueness holds; crosscheck no-op-proven then buckets hold 234/11/14 D15/E41 Σ259; 213 tests; owner exemplar = uuid 7 exact |
...
- [ ] 006: Underscore convention for `proposed_filename` (both columns; collapse ` - ` → `_`, then spaces → `_`) — PR #79 open, reviewed 2026-09-23 → **MERGE** advised; awaiting operator merge (transform invariants all proven; crosscheck buckets unchanged; disclosed 2-line report live-example regen = §4.5-compliant)
