...
- Preserve the current inventory-verified official source associations through `data/research_master_source_overrides.csv`.
  - The initial reconciliation approved **32 Veritas** and **8 Audible** URLs; later book, date-specific Satsang, and exact-title reviews add **40 Veritas** URLs, for 80 approved source overrides.
...
- *Live Your Life Like a Prayer* has two official Veritas products, now modeled in `data/product_relationships.csv`:
  - Keep the raw-ledger **November 2006** three-disc lecture-set URL as the primary master source URL for Part 1–Part 3.
  - Record the distinct **2012** one-DVD 60-minute interview as reviewed `related_material` for Part 1; it is not a replacement URL or a duplicate master item.
