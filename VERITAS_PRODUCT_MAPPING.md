...
The manual **Map Veritas Catalogue** workflow YAML writes a candidate CSV and diff artifact for review rather than auto-committing live inventory changes. Its retrying fetch code and mapping-decision input are on this branch, so merge the branch before running it manually and inspect its artifact. See [VERITAS_MAPPING_DECISIONS.md](VERITAS_MAPPING_DECISIONS.md) for the decision contract and refresh procedure.
