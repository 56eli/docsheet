...
The manual **Map Veritas Catalogue** workflow now writes a candidate CSV and diff artifact for review rather than auto-committing live inventory changes. Run it manually after merge and inspect its artifact; this session's GitHub integration cannot dispatch the workflow. See [VERITAS_MAPPING_DECISIONS.md](VERITAS_MAPPING_DECISIONS.md) for the decision contract and refresh procedure.
