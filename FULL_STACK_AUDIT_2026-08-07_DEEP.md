...
> **Historical snapshot:** this report preserves the 2026-08-07 state and intermediate counts. Use `FULL_STACK_AUDIT_2026-08-08_ARENA.md` plus generated CSV/JSON outputs for the current state.
...
- **Filename proposal disambiguation**: Resolved 2026-08-07 per owner feedback. Corrected UUID 331's series from `Volume Series` to `Books` (`data/edition_promotions.csv`), and disambiguated UUIDs 320 and 331 by assigning unique `clean_title` values (`Power vs. Force (Audiobook)` vs `Power vs. Force Audio Book`). Both editions now generate single-part filenames without `[1-2]` or `[2-2]` brackets.
